...
class EPIMRIDistortionCorrectionPush4dFastBk2:
    """
    ...
    def solve_lsq(self, yc):
        ...
        for slice_index in range(self.images.shape[2]):
            ...
            for pe_dir_index in range(self.images.shape[0]):
                ...
                indices, weights, stencil_mask, d_image, jac_slice = build_pic_stencils_2d(
                    self.omega_source[-4:],
                    self.m_source[-2:],
                    self.m_target[-2:],
                    xp[:, slice_index, :, :],
                ...
                stencil_masks.append(stencil_mask)
            ...
            stencil_masks = torch.stack(stencil_masks, dim=0)

            recon_vol_slices = []
            for vol_index in range(self.images.shape[1]):

                recon_vol_slice, res, dC = self.solve_lsq_slice(vol_index, slice_index, stencil_indices, stencil_weights, stencil_masks, d_images, diag)
    ...
    def solve_lsq_slice(self, vol_index, slice_index, stencil_indices, stencil_weights, stencil_masks, derivative_ops, diag):
        ...
        stencil_weights_masked = stencil_weights.view(-1, stencil_weights.shape[-1])
        ...
        # stencil_masks_res = stencil_masks.view(-1,stencil_masks.shape[-1]

        def A_fn(x):
            x_gathered = x[stencil_indices]  # [N, K, 1]
            Ax = (x_gathered * stencil_weights.unsqueeze(-1)).sum(dim=2)  # [N, 1]

            # Ax_expanded = Ax.expand(-1, stencil_weights.shape[1])  # [N, K]
            # contrib = stencil_weights * Ax_expanded
            contrib = Ax*stencil_weights
            # # Expand Ax to [N, K] to match weights]
            # contrib = Ax * stencil_weights  # [N, K]

            # Flatten everything
            flat_indices = stencil_indices[stencil_masks].reshape(-1)
            flat_contrib = contrib[stencil_masks].reshape(-1)  # [K*N*S, 1]

            # Scatter-add all at once
            result = torch.zeros((self.n_cells, 1), device=x.device)
            result.scatter_add_(0, flat_indices.unsqueeze(1), flat_contrib.unsqueeze(-1))
        ...
        contrib = pe_masked.unsqueeze(-1)*stencil_weights
        flat_indices = stencil_indices[stencil_masks].reshape(-1)  # [K*N*S]
        flat_contrib = contrib[stencil_masks].reshape(-1)  # [K*N*S, 1]

        # Scatter-add to get RHS
        rhs = torch.zeros((self.n_cells, 1), device=self.device)
        rhs.scatter_add_(0, flat_indices.view(-1,1), flat_contrib.view(-1,1))

        # pe_masked = pe_slices * pe_slice_masks
        # contrib = pe_masked.view(-1,1)*stencil_weights_masked
        # flat_indices = stencil_indices_masked.reshape(-1)  # [K*N*S]
        # flat_contrib = contrib.reshape(-1, 1)  # [K*N*S, 1]
        #
        # # Scatter-add to get RHS
        # rhs = torch.zeros((self.n_cells, 1), device=self.device)
        # rhs.scatter_add_(0, flat_indices[stencil_masks.view(-1)], flat_contrib[stencil_masks.view(-1)].squeeze(1))
        ...
        gathered = rhocorr[stencil_indices[stencil_masks]].squeeze(-1)  # [N_particles, K, B]
        weighted = gathered * stencil_weights[stencil_masks] # [N_particles, K, B]
...
def build_pic_stencils_2d(omega, m_source, m_target, xp, do_derivative=False, return_jacobian=False):
    ...
    J[invalid_idx] = -1
    B[invalid_idx] = 0
    stencil_mask = ~invalid_idx

    stencil_size = nbx * nby

    indices = I.view(stencil_size, n_cells)
    weights = B.view(stencil_size, n_cells)
    stencil_mask = stencil_mask.view(stencil_size, n_cells)
    indices = indices.permute(1, 0)
    weights = weights.permute(1, 0)
    stencil_mask = stencil_mask.permute(1,0)

    results = [indices, weights, stencil_mask]
