...
class EPIMRIDistortionCorrectionPush4dFast:
    """
    ...
    def __init__(self,
        ...
        if PC is not None:
            ...
            self.PC = None
        self.solver = PCG(max_iter=200, tol=1e-5, verbose=False)
    ...
    def solve_lsq(self, yc):
        ...
        for slice_index in range(self.images.shape[2]):
            ...
            for pe_dir_index in range(self.images.shape[0]):
                ...
                # chk_image = create_diagnostic_image(self.m_source[2:], self.omega_source[2:], device=self.device, dtype=self.dtype)
                # chk_image = chk_image.reshape(-1,1)
                # # Gather cell values at stencil indices for each particle
                # # I_gathered = chk_image[
                # #     js]  # [n_particles, K, 1] (broadcastable)
                #
                # I_gathered = chk_image[
                #     js]  # [n_particles, K, 1] (broadcastable)
                # I_weighted = I_gathered.squeeze(
                #     -1) * weights  # [n_particles, K]
                #
                # result = torch.zeros((self.n_cells, 1), device=chk_image.device)
                # result.scatter_add_(0, indices.reshape(-1,1), I_weighted.reshape(-1,1))
                # # I_particle = I_weighted.sum(dim=1,
                # #                             keepdim=True)  # [n_particles, 1]
                #
                # # contrib = (chk_image.reshape(-1,1)*weights).sum(dim=1)
                # # flat_contrib = contrib.flatten()
                ...
                # #
                # save_data(result.reshape(-1, *self.m_target[2:]),
                #           f"/home/laurin/workspace/PyHySCO/data/results/debug/check_{pe_dir_index}.nii.gz")
    ...
    def solve_lsq_slice(self, vol_index, slice_index, stencil_indices, stencil_weights, derivative_ops, diag):
        ...
        def A_fn(x):
            ...
            val_idx = (flat_indices >= 0) & (flat_indices < self.n_cells)
            flat_contrib = flat_contrib[val_idx]
            flat_indices = flat_indices[val_idx]
        ...
        contrib = pe_masked.unsqueeze(-1) * stencil_weights  # [K*N, S]
        ...
        for pe_index in range(pe_slices.shape[0]):
            ...
            # save_data(dx_weighted.reshape(-1, *self.m_source[2:]).permute(1, 2, 0),
            #           f"/home/laurin/workspace/PyHySCO/data/results/debug/dx_{pe_index}.nii.gz")
            # save_data(dy_weighted.reshape(-1, *self.m_source[2:]).permute(1, 2, 0),
            #           f"/home/laurin/workspace/PyHySCO/data/results/debug/dy_{pe_index}.nii.gz")
