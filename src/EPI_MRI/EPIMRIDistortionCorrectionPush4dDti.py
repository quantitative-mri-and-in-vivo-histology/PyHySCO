...
class EPIMRIDistortionCorrectionPush4dDti:
	"""
	...
	def __init__(self,
		...
		Lx, Ly = get_2d_gradient_matrix(self.m_target[-2],
										self.m_target[-1], self.device,
										self.dtype)
	...
	def solve_lsq(self, yc):
		...
		for slice_index in range(dti_images_lin.shape[1]):
			...
			for image_index in range(dti_images_lin.shape[0]):
				indices, weights, d_image, jac_slice = build_pic_stencils_2d(self.dataObj.omega[-4:],
												 self.m_source[-2:],
												 self.m_target[-2:],
			...
			valid_mask = (obs_image < 1.0) & obs_masks & torch.isfinite(
				obs_image)
			...
			def A_fn(x):
				...
				for i in range(obs_image.shape[0]):
					...
					AtAx_i = apply_pic_tensor_rmatvec(stencil_filtered, Ax_i,
													  Q_i, self.m_target[-2] *
													  self.m_target[-1])
					result += AtAx_i
			...
			with torch.no_grad():
				rhs = torch.zeros(
					(6 * self.m_target[-2] * self.m_target[-1], 1),
					device=self.device)
				...
				for i in range(obs_image.shape[0]):
					...
					Atb_i = apply_pic_tensor_rmatvec(
						...
						self.m_target[-2] * self.m_target[-1]
					)
					rhs += Atb_i
			...
			diag_data = torch.zeros((6 * self.m_target[-2] * self.m_target[-1]),
									device=self.device)

			for i in range(dti_images_lin.shape[0]):
				Q_i = Q[i]  # shape (6,)
				stencil = roi_stencils[i]
				diag_data += compute_diag_ATA_tensor(stencil, Q_i,
													 self.m_target[-2] *
													 self.m_target[-1])

			precond_diag = diag_data + lambda_smooth * self.LxT_Lx

			M = make_diag_preconditioner(precond_diag)


			solver = PCG(max_iter=200, tol=3e-2, verbose=True)
			...
			D_raw = rhocorr.reshape(6, self.m_target[-2], self.m_target[-1])
			# r = r.permute(2,0,1)
