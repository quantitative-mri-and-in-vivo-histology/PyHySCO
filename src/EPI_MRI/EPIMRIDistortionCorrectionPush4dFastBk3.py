...
class EPIMRIDistortionCorrectionPush4dFastBk3:
    """
    ...
    def __init__(self,
        ...
        self.n_cells = torch.prod(self.m_source[2:]).long()
        self.n_particles = torch.prod(self.m_target[2:]).long()
    ...
    def solve_lsq(self, yc):
        ...
        for slice_index in range(self.images.shape[2]):
            ...
            for pe_dir_index in range(self.images.shape[0]):
                ...
                diag += compute_diag_ATA((indices, weights), self.n_particles)
    ...
    def solve_lsq_slice(self, vol_index, slice_index, stencil_indices, stencil_weights, stencil_masks, derivative_ops, diag):
        ...
        def A_fn(x):
            ...
            valid_idx = (flat_indices >= 0) & (flat_indices < self.n_particles)
            flat_indices = flat_indices[valid_idx]
            flat_contrib = flat_contrib[valid_idx]

            # Scatter-add all at once
            result = torch.zeros((self.n_particles, 1), device=x.device)
        ...
        rhs = torch.zeros((self.n_particles, 1), device=self.device)
