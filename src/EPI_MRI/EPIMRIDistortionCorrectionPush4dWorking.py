...
class EPIMRIDistortionCorrectionPush4dWorking:
	"""
	...
	def __init__(self,
		...
		self.D_vol = derivative_operator(self.dataObj.omega[-6:], self.dataObj.m[-3:], self.dtype, self.device)
	...
	def eval(self, yc, yref=None, do_derivative=False, calc_hessian=False):
		...
		for vol_index in range(self.dataObj.m[0]):
			...
			for slice_index in range(self.dataObj.m[1]):
				...
				lambda_smooth = 0.01  # or any other value you like
		...
		Jac_mean = Jac_all.mean(dim=0).reshape(*self.target_m[1:])
		...
		dbc = self.D.mat_mul(yc)  # derivative matrix & derivative of deformation vector
		...
		Dc = sse/len(self.dataObj.image_pairs)
		Jc = hd_target * Dc + hd * self.alpha * Sc + hd*self.beta*Pc
		results = [Jc]

		if do_derivative:
			dP = self.D_vol.transp_mat_mul(dG)
			dJ = hd_target * dD + hd * self.alpha * dS + hd*self.beta*dP
