from EPI_MRI.LeastSquaresCorrectionMultiPeSparse4dDti import \
    LeastSquaresCorrectionMultiPeSparse4dDti
...
def load_bvec_bval_as_tensors(nifti_filename, device='cpu', dtype=torch.float32):
    """
    Loads the corresponding .bvec and .bval files for a given .nii/.nii.gz file.
    ...
    device : str
        Torch device to load tensors on.
    dtype : torch.dtype
        Data type for tensors.

    Returns
    -------
    bvec : torch.Tensor of shape (3, N)
        Gradient directions.
    bval : torch.Tensor of shape (N,)
        b-values.
    """
    # Strip extension to get base path
    if nifti_filename.endswith('.nii.gz'):
        base = nifti_filename[:-7]
    elif nifti_filename.endswith('.nii'):
        base = nifti_filename[:-4]
    else:
        raise ValueError("Expected filename to end with .nii or .nii.gz")

    bvec_path = base + '.bvec'
    bval_path = base + '.bval'

    if not os.path.exists(bvec_path) or not os.path.exists(bval_path):
        raise FileNotFoundError(f"Could not find corresponding .bvec or .bval for {nifti_filename}")

    bvec = torch.tensor(np.loadtxt(bvec_path), dtype=dtype, device=device)
    bval = torch.tensor(np.loadtxt(bval_path), dtype=dtype, device=device)

    # Ensure shapes: bvec should be (3, N), bval should be (N,)
...
class MultiPeDataObject:
    def __init__(self, image_config, device='cpu', dtype=torch.float64, pair_idx=None, first_vol_only=False, normalize=False):
    ...
    def load_data(self):
        ...
        bvec, bval = load_bvec_bval_as_tensors(
            os.path.join(image_dir, image_config['pe_pairs'][0][0]), device=device, dtype=dtype)
        ...
        for image_pair_index, image_path_pair in enumerate(image_config['pe_pairs']):
            ...
            if self.first_vol_only is False:
                rho0_b0 = rho0[bval < 50]
                rho1_b0 = rho1[bval < 50]
                rho0_b0_mean = torch.mean(rho0_b0,0)
                rho1_b0_mean = torch.mean(rho1_b0,0)
                rho0_dti_shell = rho0[dti_idx]
                rho1_dti_shell = rho1[dti_idx]
                rho0 = rho0_dti_shell / rho0_b0_mean
                rho1 = rho1_dti_shell / rho1_b0_mean
                m[0] = rho0.shape[0]
...
def main():
    ...
    opt = LeastSquaresCorrectionMultiPeSparse4dDti(data, avg_operator)
    ...
    save_data(corrected_image.reshape([6, 2, data.m[2], data.m[3]]).permute(data.permute_back[0]),
                os.path.join(args.output_dir, 'imCorrected.nii.gz'))
