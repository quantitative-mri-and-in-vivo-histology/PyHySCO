...
from EPI_MRI.EPIMRIDistortionCorrectionPush4dProper import *
...
def main():
    ...
    loss_func = EPIMRIDistortionCorrectionPush4dProper(
        data,
        alpha=args.alpha,
        beta=args.beta,
    ...
    scaling[:2] = data.m[2:] / torch.tensor(target_res[2:], device=data.m.device, dtype=torch.float32)

    target_affine = rescale_affine(data.mats[0], scaling)
    target_affine = target_affine.cpu().numpy()

    save_data(corrected_image.reshape(list(loss_func.m_distorted)).permute(data.permute_back[0]),
              os.path.join(args.output_dir, 'imCorrected.nii.gz'), target_affine)

    save_data(
        opt.Bc.reshape(list(m_plus(target_res[1:].cpu()))).permute(
            opt.corr_obj.dataObj.permute[0][1:]),
        os.path.join(args.output_dir, 'EstFieldMap.nii.gz'), data.mats[0].cpu().numpy())
    save_data(
        opt.B0.reshape(list(m_plus(target_res[1:].cpu()))).permute(
            opt.corr_obj.dataObj.permute[0][1:]),
