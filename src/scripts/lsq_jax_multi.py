...
def build_pic_stencils_2d(omega, m_source, m_target, xp, return_jacobian=False):
    ...
    # nVoxel = nbx * nby
    #
    # I = jnp.empty(nVoxel * n_cells, dtype=jnp.int64)
    # J = jnp.empty(nVoxel * n_cells, dtype=jnp.int64)
    # B = jnp.empty(nVoxel * n_cells, dtype=xp.dtype)
    ...
    for i, px in enumerate(range(-int(pwidth[0]), int(pwidth[0]) + 1)):
        for j, py in enumerate(range(-int(pwidth[1]), int(pwidth[1]) + 1)):
            ...
            # I.at[idx].set(Iij)
            # J.at[idx].set(jnp.arange(n_cells))
            # B.at[idx].set(Bij)
            I.append(Iij)
...
# def pic_forward(rho: jnp.ndarray, indices: jnp.ndarray, weights: jnp.ndarray, n_cells) -> jnp.ndarray:
#     """Compute  y = T rho  using gathered indices / weights.
#
#     rho        : (N,)           – particles (flattened)
#     indices    : (N, K) int32   – voxel id per stencil weight
...
#     returns    : (N,)           – voxel values per particle (actually length N)
#     """
#     y_gathered = rho[indices]
#     contrib = (y_gathered * weights)
#     idx = indices.reshape(-1)
#     val = contrib.reshape(-1)
#     z = jnp.zeros(n_cells, rho.dtype)
#     z = z.at[idx].add(val)
#     return z
#
# def pic_adjoint(y: jnp.ndarray, indices: jnp.ndarray, weights: jnp.ndarray) -> jnp.ndarray:
#     """Compute z = Tᵀ y.
#
#     y          : (N,)           – values at particle sites
#     indices    : (N, K) int32   – voxel indices
...
#     """Return   diag(Tᵀ T)   to be used as a Jacobi PC."""
#
#     diag = jnp.zeros(n_particles, dtype=weights.dtype)
#     for k in range(weights.shape[0]):
#         w2 = jnp.sum(weights[k] ** 2, axis=[1])         # (N,)  per particle sum
#         diag = diag.at[indices[k]].add(w2)
#     return jnp.maximum(diag, 1e-6)
...
def tikhonov_like_torch(H, W):
    def apply(r):
        U = r.reshape(H, W)

        # forward diff to the right / down
        dx = jnp.roll(U, -1, axis=1) - U
        dy = jnp.roll(U, -1, axis=0) - U
        dx = dx.at[:, -1].set(0.0)   # omit last col
        dy = dy.at[-1, :].set(0.0)   # omit last row

        # adjoint: negative forward diff
        dxt = -(dx - jnp.roll(dx, 1, axis=1))
        dyt = -(dy - jnp.roll(dy, 1, axis=0))
        dxt = dxt.at[:, 0].set(0.0)  # first col had no neighbour
        dyt = dyt.at[0, :].set(0.0)  # first row had no neighbour
...
def solve_lsq_slice(
    ...
    n_cells = m_distorted[0]*m_distorted[1]

    lap_fn = tikhonov_like_torch(m_recon[0], m_recon[1])
    ...
    rho_est = pcg(Ax, rhs, M_inv, rho_init, tol=1e-8, maxiter=20)
    # rho_est = cg(Ax, rhs, M=M_inv, x0=rho_init, tol=1e-8, maxiter=20)
    return rho_est
...
def solve_one_slice(vol_slice_obs,             # (n_obs, H, W)  – PE/RPE stack
    ...
    for xp_grid in xp:
        ...
        idx, w = build_pic_stencils_2d(omega_recon, m_recon, m_distorted, xp_grid)
        idx_all.append(idx)
        w_all.append(w)
    ...
    # ------------------------------------------------------------------
    rho_init = jnp.zeros(m_recon[-2:], dtype=y.dtype).reshape(-1)
...
def batch_solve(observations, omega_recon, m_recon, m_distorted, xp):
    ...
    for slice_index, ob in enumerate(observations):
        ...
        xp_slice_flat = xp_slice.reshape(*xp_slice.shape[0:2], -1)
        solve_fn = lambda obs: solve_one_slice(obs, omega_recon, m_distorted, m_recon, xp_slice_flat)
...
if __name__ == "__main__":
    ...
    for pair_index, pair in enumerate(data.image_pairs):
        ...
        xp_lin = xp_lin[1:, :].reshape(2, *target_res[1:])
    ...
    recon = batch_solve(dwi_images,
                        omega_recon=jnp.array(omega[4:], dtype=jnp.float32),
