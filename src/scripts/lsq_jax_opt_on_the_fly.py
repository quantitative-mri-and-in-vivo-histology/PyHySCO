...
class DistortionModel:
    ...
    def apply(self, xp_base, obs_index, volume_index):
        ...
        xp_lin = xp_pe + self.phase_signs[obs_index, volume_index] * bc.reshape(3, -1)
...
class DwiOptimizer:
    ...
    # def solve(self, vol_obs, distortion_model):
    #     """
    ...
    #     rho_est = []
    #     total_loss = 0.0
    #     for vol_index in range(vol_obs.shape[1]):
    #         img, loss = self._solve_vol(vol_obs, distortion_model, vol_index)
    #         rho_est.append(img)
    #         total_loss += loss
    #
    #     rho_est = jnp.stack(rho_est, axis=0)
    #
    #     return rho_est, total_loss
    ...
    def ATAx(self, x, vol_index, n_obs, lambda_smooth, distortion_model):
        ...
        def single_TtT(k):
            ...
            # idx, w = build_pic_stencils_3d(self.omega, self.m_cell, self.m_part,
            #                                xp)
            idx, w = self.stencil_build_fn(xp)
    ...
    # def _solve_vol(self, vol_obs, distortion_model, vol_index):
    #     """
    #     Runs the PIC-LSQ solver for *one* (volume, slice) pair.
    #     """
    #
    #     y = vol_obs[:, vol_index].reshape(vol_obs.shape[0], -1)
    #     rho_init = jnp.zeros(self.m_part[-3:], dtype=y.dtype).reshape(-1)
    #
    #     # M_inv = lambda r: r / diag
    #     M_inv = lambda r: r
    #
    #     # Ax = lambda x: pic_normal_matvec(x, self.xp_base, self.omega, self.m_part, self.m_cell,
    #     #                                  self.n_cells,
    #     #                                  self.lambda_smooth)
    #
    #     # def apply(self, xp_base, obs_index, vol_index):
    #     #     return apply_distortion_model(
    #     #         xp_base,
    #     #         self.bc,
    #     #         self.thetas[obs_index, vol_index],
    #     #         self.phase_signs[obs_index, vol_index],
    #     #         self.pe_mats[obs_index, vol_index],
    #     #         self.pe_mats[0][0],  # assuming constant reference
    #     #         self.omega
    #     #     )
    #
    #     ATAx_func = lambda x: self.ATAx(x, vol_index, n_obs, lambda_smooth=self.lambda_smooth, distortion_model=distortion_model)
    #
    #     rhs = self.ATy(y, vol_index, distortion_model=distortion_model)
    #
    #     rho_est, _ = cg(ATAx_func, rhs, M=M_inv, x0=rho_init, tol=1e-8, maxiter=10)
