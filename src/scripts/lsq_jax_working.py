...
def solve_lsq_slice(
    ...
    rho_est = pcg(Ax, rhs, M_inv, rho_init, tol=1e-5, maxiter=200)
    return rho_est
...
def solve_one_slice(vol_slice_obs,             # (n_obs, H, W)  – PE/RPE stack
                    omega,                     # length-4 array for this slice
                    m_src,
                    m_tgt,              # (2,) – distorted & recon grids
                    xp,
                    lambda_smooth=0.1):
    """
    ...
    # xp = get_cell_centered_grid(omega, m_tgt, return_all=True)
    # xp = xp.reshape(2, H, W)


    idx, w = build_pic_stencils_2d(omega, m_src[1:], m_tgt[1:], xp)
    ...
    rho_hat  = solve_lsq_slice(rho_init, y, idx, w, H, W,
                               lambda_smooth=lambda_smooth)
    return rho_hat


# @jax.jit
def batch_solve(observations, omega, m_distorted, m_recon, xp):
    ...
    for slice_index, ob in enumerate(observations):
        ...
        for vol_index, vol in enumerate(ob):
            results_per_vol.append(solve_one_slice(vol, omega, m_distorted, m_recon, xp_slice_flat))
...
if __name__ == "__main__":
    ...
    xp = get_cell_centered_grid(omega, m_recon[1:], return_all=True)
    xp = xp.transpose(1,0)
    ...
    recon = batch_solve(dwi_images,
                        omega=jnp.array(data.omega[4:], dtype=jnp.float32),
                        m_distorted=m_distorted,
                        m_recon=m_recon,
                        xp=xp_offset)

    recon = recon.reshape(*recon.shape[0:2], 128, 128)
