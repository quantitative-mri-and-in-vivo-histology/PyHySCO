...
class MultiPeDataObject:
    def __init__(self, image_config, device='cpu', dtype=torch.float64, pair_idx=None, do_normalize=True, first_vol_only=False):
        ...
        if do_normalize:
            for i in range(len(self.image_pairs)):
                self.image_pairs[i].pe_image, self.image_pairs[i].rpe_image = normalize(self.image_pairs[i].pe_image, self.image_pairs[i].rpe_image)
    ...
    def load_data(self):
        ...
        pe_rpe_permute_back = []
        pe_rpe_permute = []
        n_slice_idx = list(range(10,15))
        n_vol_idx = list(range(0,5))
        n_vol_idx = None
...
def main():
    ...
    parser.add_argument("--initialization", default=InitializeCFMultiPeSimple, help="Initialization method to use (default=InitializeCFMultiPeSimple)")
    ...
    loss_func = EPIMRIDistortionCorrectionLsq4d(data, alpha=args.alpha, beta=args.beta, averaging_operator=args.averaging, derivative_operator=args.derivative, regularizer=args.regularizer, rho=args.rho, PC=args.PC)
