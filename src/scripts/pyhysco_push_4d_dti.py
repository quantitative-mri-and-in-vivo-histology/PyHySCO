from EPI_MRI.EPIMRIDistortionCorrectionPush4dDti import \
    EPIMRIDistortionCorrectionPush4dDti
...
def main():
    ...
    pair_idx = [0]
    # pair_idx = 0
    data = MultiPeDtiData(args.image_config, device=device, dtype=dtype, pair_idx=pair_idx, do_normalize=False)
    ...
    target_res = [128,128]
    loss_func = EPIMRIDistortionCorrectionPush4dDti(
        data,
        alpha=args.alpha,
        beta=args.beta,
    ...
    opt = args.optimizer(loss_func, max_iter=0, verbose=True,
    ...
    save_data(corrected_image.reshape(list(loss_func.target_m)).permute(data.permute_back[0]),
