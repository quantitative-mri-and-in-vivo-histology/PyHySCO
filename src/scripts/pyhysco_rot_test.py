...
def main():
    ...
    for i, image_pair in enumerate(data.image_pairs):
        ...
        # Extract rotation part using SVD
        U, _, Vt = torch.linalg.svd(data.rel_mats[i][:3, :3])
        R = U @ Vt
        ...
        zz, yy, xx = torch.meshgrid(z, y, x, indexing='ij')
        grid_voxel = torch.stack((xx, yy, zz), dim=-1)  # (D,H,W,3)

        # 2. Center, rotate, uncenter
        center = torch.tensor([(W-1)/2, (H-1)/2, (D-1)/2], device=device, dtype=dtype)
        grid_centered = grid_voxel - center
        grid_rotated = grid_centered @ R.T + center

        # 3. Normalize to [-1, 1] for grid_sample
        grid_rotated[..., 0] = 2.0 * grid_rotated[..., 0] / (W - 1) - 1
        grid_rotated[..., 1] = 2.0 * grid_rotated[..., 1] / (H - 1) - 1
        grid_rotated[..., 2] = 2.0 * grid_rotated[..., 2] / (D - 1) - 1

        # 4. Add batch dimension
        grid_rotated = grid_rotated.unsqueeze(0)  # (1,D,H,W,3)

        # grid = create_normalized_grid(data.m, device=device, dtype=dtype)
        # center = torch.tensor([(data.m[0]-1)/2, (data.m[1]-1)/2, (data.m[2]-1)/2], device=device, dtype=dtype)
        # grid_centered = grid - center
        # grid_rot = grid_centered @ R.T + center  # Rotate around center

        # grid = grid.unsqueeze(0)  # add batch dimension
        # # grid_flat = grid.view(-1, 3)  # (N, 3)
        # # grid_rot = grid_flat @ R.T  # (N, 3)
        # grid_rot = grid_rot.view(grid.shape)  # (1, D, H, W, 3)
        
        # Transform images
        image_pair.pe_image = F.grid_sample(
            image_pair.pe_image.unsqueeze(0).unsqueeze(0),
            grid_rotated,
            mode='bilinear',
            padding_mode='border',
            align_corners=True
        ...
        image_pair.rpe_image = F.grid_sample(
            image_pair.rpe_image.unsqueeze(0).unsqueeze(0),
            grid_rotated,
            mode='bilinear',
            padding_mode='border',
            align_corners=True
